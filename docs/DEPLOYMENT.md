...
bash packaging/build-dist.sh 1.78.4 ghcr.io/awijesundara/serviceops-server sha256:<pushed-image-digest>
# Tag-pinned fallback (prints a warning; the tag can later be overwritten):
# bash packaging/build-dist.sh 1.78.4 <your-registry>/serviceops
bash packaging/build-rpms.sh 1.78.4
```
...
sudo dnf install ./serviceops-1.78.4-1.el$(rpm -E %rhel).noarch.rpm
