...
Status: current deployment 1.97.2; transient activity UI, multiple-service privacy routing, latest-incident retrieval, readiness, and live self-hosted streaming are verified. Commercial live inference remains unverified. AI-05 implementation has started in the working tree and is not deployed. Estimates are engineering effort, not promised calendar delivery.
...
| AI-05 Approved actions | Days 11–15; depends on AI-04 quality review | Exact proposed change; expiring approval bound to record version; reauthorize; existing mutation APIs; audit/idempotency | No approval bypass, stale action rejection, no duplicate mutation | In progress: ticket-comment action implemented locally; state/assignment/priority actions remain planned |
